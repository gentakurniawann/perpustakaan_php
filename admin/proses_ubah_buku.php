    ...
    if ($_POST) {
        ...
        if (empty ($nama_buku)) {
            ...
            if($file_size < 2048000 and ($file_type == "image/jpeg" or $file_type== "image/png")){
                ...
                if (empty($file_name)){
                    $update= mysqli_query ($koneksi, "update buku set nama_buku='".$nama_buku."', pengarang='".$pengarang."', deskripsi='".$deskripsi."' where id_buku='".$id_buku."' ") or die (mysqli_error($koneksi));
                    if ($update) {
                    ...
                    $update= mysqli_query ($koneksi, "update buku set nama_buku='".$nama_buku."', pengarang='".$pengarang."', deskripsi='".$deskripsi."', foto='".$file_name."' where id_buku='".$id_buku."' ") or die (mysqli_error($koneksi));
                    if ($update) {
                        echo "<script> alert ('Sukses update buku'); location.href='tampil_buku.php' ; </script>";  
                    }else{
                        echo "<script> alert ('Gagal update buku'); location.href='tampil_buku.php' ; </script>";
